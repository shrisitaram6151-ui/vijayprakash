...
export default async function StatusPage({
  ...
  if (orderId) {
    try {
      ...
      paid = status === "PAID";
      notFound = false;
      const newStatus = paid ? "paid" : status === "ACTIVE" ? "pending" : "failed";
        ...
        <div className="card-dark w-full rounded-3xl p-8 text-center">
          {paid ? (
            <>
              ...
              <h1 className="mt-4 font-display text-2xl font-bold gold-text">भुगतान सफल!</h1>
            ...
            <>
              ...
              <p className="mt-3 text-amber-100/80">
                {notFound ? "कृपया दोबारा प्रयास करें।" : `भुगतान की स्थिति: ${status}।`}
              </p>
              <div className="mt-6 flex flex-wrap justify-center gap-3">
                <Link href="/booking" className="rounded-full gold-gradient px-5 py-2.5 font-semibold text-[#3a2a05]">पुनः प्रयास करें</Link>
                <a href={telLink(SITE.helpline)} className="rounded-full border border-amber-400/40 px-5 py-2.5 font-semibold text-amber-200">Helpline: {SITE.helpline}</a>
              </div>
            </>
          )}
          <div className="mt-8">
            <Link href="/" className="text-sm text-amber-300 underline">मुख्य पृष्ठ पर लौटें</Link>
          </div>
        </div>
      </main>
    </>
